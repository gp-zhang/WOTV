...
+ (void)showWithStatusWithoutImage:(NSString *)string afterDelay:(NSTimeInterval)seconds {
    [SVProgressHUD show];
    [[SVProgressHUD sharedView] dismissWithStatusWithoutImage:string afterDelay:seconds];
}
...
- (void)dismissWithStatusWithoutImage:(NSString *)string afterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        if(self.alpha != 1)
            return;
        ...
        [UIView animateWithDuration:0.15
                         ...
                         completion:^(BOOL finished){ 
                             if(self.alpha == 0) {
                                 ...
                                 //add at 2012.12.7 for dissmis keyboard when remove。
